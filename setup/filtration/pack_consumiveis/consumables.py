...
"")    
        ...
        self.table_employee.setHorizontalHeaderLabels(["Ref","Consumable","Stock Quantity"," "," "])
        ...
        list_consumable = controller.listar()
        self.table_employee.setRowCount(10)
        tablerow = 0

        for row in list_consumable:
            
            self.btn_edit_employee = QtWidgets.QPushButton("Edit Consumable")
            ...
            self.btn_remove_employee = QtWidgets.QPushButton("Remove Consumable")
            ...
            self.table_employee.setItem(tablerow,0,QtWidgets.QTableWidgetItem(str(row[0])))
            self.table_employee.setItem(tablerow,1,QtWidgets.QTableWidgetItem(str(row[1])))
            self.table_employee.setItem(tablerow,2,QtWidgets.QTableWidgetItem(str(row[2])))
        ...
        def show_form_edit_engier(id,nome,quant):
            self.window = QtWidgets.QMainWindow()
            import filtration.pack_consumiveis.consumables_edit as edit
            self.ui = edit.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text(),id,nome,quant)
            self.window.show()
            MainWindow.close()
        ...
        def btn_clicked_edit():
            ...
            if isinstance(self.btn_edit_employee, QtWidgets.QPushButton):
                ...
                if (item_id is not None ) or (item_nome is not None):
                    ...
                    print(id,nome,quant)
                    show_form_edit_engier(id,nome,quant)
    ...
    def retranslateUi(self, MainWindow):
        ...
        def show_form_add_consumable():
            self.window = QtWidgets.QMainWindow()
            import filtration.pack_consumiveis.consumables_add
            self.ui = filtration.pack_consumiveis.consumables_add.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text())
            self.window.show()
            MainWindow.close()
