...
"")
        ...
        self.table_ppe = QtWidgets.QTableWidget(self.frame)
        self.table_ppe.setGeometry(QtCore.QRect(20, 320, 1091, 421))
        self.table_ppe.setAutoFillBackground(True)
        self.table_ppe.setStyleSheet("QTableWidget{\n"
"\n"
"\n"
"border:none;\n"
...
"")    
        ...
        self.table_ppe.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.table_ppe.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.table_ppe.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        self.table_ppe.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
        self.table_ppe.setTextElideMode(QtCore.Qt.ElideMiddle)
        self.table_ppe.setShowGrid(False)
        self.table_ppe.setGridStyle(QtCore.Qt.NoPen)
        ...
        self.table_ppe.horizontalHeaderItem(3).setTextAlignment(0x0001)
        self.table_ppe.horizontalHeaderItem(4).setTextAlignment(0x0001)

        list_ppe = tank_cleanning.pack_pipe.pipeController.listar_table()
        self.table_ppe.setRowCount(10)
        tablerow = 0

        for row in list_ppe:
            
            self.btn_edit_ppe = QtWidgets.QPushButton("Edit PPE")
            self.btn_edit_ppe.setStyleSheet("\n" "\n" "QPushButton#btn_edit_ppe{\n" "\n" "border:none;\n" "background-color:#044e42;\n" "color:white;\n" "font-size:13px;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "text-align:rigth;\n" "}\n" "\n" "QPushButton#btn_edit_ppe:hover{\n" " background-color: #044e42;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "}\n" "\n" "QPushButton#btn_edit_ppe:pressed {\n" " background-color: #044e42;\n" "border-radius: 0px;\n" "background-color: #033029;\n" "padding:5px;\n" " }\n" "")
            self.btn_edit_ppe.setIcon(icon_edit)
            self.btn_edit_ppe.setObjectName("btn_edit_ppe")

            self.btn_remove_ppe = QtWidgets.QPushButton("Remove PPE")
            self.btn_remove_ppe.setStyleSheet("\n" "\n" "QPushButton#btn_remove_ppe{\n" "\n" "border:none;\n" "background-color:#044e42;\n" "color:white;\n" "font-size:13px;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "text-align:rigth;\n" "}\n" "\n" "QPushButton#btn_remove_ppe:hover{\n" " background-color: #044e42;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "}\n" "\n" "QPushButton#btn_remove_ppe:pressed {\n" " background-color: #044e42;\n" "border-radius: 0px;\n" "background-color: #033029;\n" "padding:5px;\n" " }\n" "")
            self.btn_remove_ppe.setIcon(icon_delete)
            ...
            self.btn_edit_ppe.clicked.connect(lambda:btn_clicked_edit())
            self.btn_remove_ppe.clicked.connect(lambda:btn_clicked_delete())
        ...
        def btn_clicked_edit():
            self.btn_edit_ppe = self.btn_edit_ppe.sender()
            if isinstance(self.btn_edit_ppe, QtWidgets.QPushButton):
                index = self.table_ppe.indexAt(self.btn_edit_ppe.pos())
                row = index.row()
                col = index.column()
                ...
                item_quantidade = self.table_ppe.item(row,col_email)
        ...
        def btn_clicked_delete():
            self.btn_remove_ppe = self.btn_remove_ppe.sender()
            if isinstance(self.btn_remove_ppe, QtWidgets.QPushButton):

                index = self.table_ppe.indexAt(self.btn_remove_ppe.pos())
                row = index.row()
                col = index.column()
        ...
        self.table_ppe.horizontalHeader().setDefaultSectionSize(217)
        self.table_ppe.horizontalHeader().setHighlightSections(False)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_titulo_form.setText(_translate("MainWindow", "PPEs"))
        self.lbl_text_form.setText(_translate("MainWindow", "Below is the list of all  PPEs registered in the system"))
       
        self.btn_add_consumable.setText(_translate("MainWindow", "Add PPE "))
        self.btn_add_consumable.clicked.connect(lambda: show_form_add_ppe())
        ...
        def show_form_add_ppe():
            self.window = QtWidgets.QMainWindow()
            import tank_cleanning.pack_pipe.ppe_add
            self.ui = tank_cleanning.pack_pipe.ppe_add.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text())
            self.window.show()
            MainWindow.close()
