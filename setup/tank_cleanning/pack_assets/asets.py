...
import tank_cleanning.pack_assets.assetsController
...
"")
        ...
        self.table_assets = QtWidgets.QTableWidget(self.frame)
        self.table_assets.setGeometry(QtCore.QRect(20, 320, 1091, 421))
        self.table_assets.setAutoFillBackground(True)
        self.table_assets.setStyleSheet("QTableWidget{\n"
"\n"
"\n"
"border:none;\n"
...
"")    
        ...
        self.table_assets.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.table_assets.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.table_assets.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        self.table_assets.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
        self.table_assets.setTextElideMode(QtCore.Qt.ElideMiddle)
        self.table_assets.setShowGrid(False)
        self.table_assets.setGridStyle(QtCore.Qt.NoPen)
        self.table_assets.setCornerButtonEnabled(False)
        self.table_assets.setObjectName("table_assets")
        self.table_assets.setColumnCount(5) 
        self.table_assets.setHorizontalHeaderLabels(["Assets","Stock Quantity"," "," "," "])
        self.table_assets.horizontalHeaderItem(0).setTextAlignment(0x0001)
        self.table_assets.horizontalHeaderItem(1).setTextAlignment(0x0001)
        self.table_assets.horizontalHeaderItem(2).setTextAlignment(0x0001)
        self.table_assets.horizontalHeaderItem(3).setTextAlignment(0x0001)
        self.table_assets.horizontalHeaderItem(4).setTextAlignment(0x0001)

        list_asset = tank_cleanning.pack_assets.assetsController.listar_table()
        self.table_assets.setRowCount(10)
        tablerow = 0

        for row in list_asset:
            
            self.btn_edit_asset = QtWidgets.QPushButton("Edit Asset")
            self.btn_edit_asset.setStyleSheet("\n" "\n" "QPushButton#btn_edit_asset{\n" "\n" "border:none;\n" "background-color:#044e42;\n" "color:white;\n" "font-size:13px;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "text-align:rigth;\n" "}\n" "\n" "QPushButton#btn_edit_asset:hover{\n" " background-color: #044e42;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "}\n" "\n" "QPushButton#btn_edit_asset:pressed {\n" " background-color: #044e42;\n" "border-radius: 0px;\n" "background-color: #033029;\n" "padding:5px;\n" " }\n" "")
            self.btn_edit_asset.setIcon(icon_edit)
            self.btn_edit_asset.setObjectName("btn_edit_asset")

            self.btn_remove_employee = QtWidgets.QPushButton("Remove Asset")
            ...
            self.table_assets.setItem(tablerow,0,QtWidgets.QTableWidgetItem(row[1]))
            self.table_assets.setItem(tablerow,1,QtWidgets.QTableWidgetItem(row[2]))
            self.table_assets.setItem(tablerow,2,QtWidgets.QTableWidgetItem(row[0]))
            self.table_assets.setCellWidget(tablerow,3,self.btn_edit_asset)
            self.table_assets.setCellWidget(tablerow,4,self.btn_remove_employee)

            self.btn_edit_asset.clicked.connect(lambda:btn_clicked_edit())
        ...
        def show_form_edit_asset(name,email):
            self.window = QtWidgets.QMainWindow()
            import tank_cleanning.pack_assets.asets_edit as edit
            self.ui = edit.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text(),name,email)
            self.window.show()
            MainWindow.close()

        def show_form_list_asset():
            self.window = QtWidgets.QMainWindow()
        ...
        def btn_clicked_edit():
            self.btn_edit_asset = self.btn_edit_asset.sender()
            if isinstance(self.btn_edit_asset, QtWidgets.QPushButton):
                index = self.table_assets.indexAt(self.btn_edit_asset.pos())
                row = index.row()
                col = index.column()
                ...
                item_position = self.table_assets.item(row,col_position)
                
                item = self.table_assets.item(row,col)

                item_quantity = self.table_assets.item(row,col_email)
                
                if (item is not None ) or (item_position is not None):
                    
                    name = item.text()
                    quantity = item_quantity.text()
                    show_form_edit_asset(name,quantity)
        ...
        def btn_clicked_delete():
            ...
            if isinstance(self.btn_remove_employee, QtWidgets.QPushButton):
                ...
                index = self.table_assets.indexAt(self.btn_remove_employee.pos())
                row = index.row()
                col = index.column()
                ...
                item_email = self.table_assets.item(row,col_email)
                ...
                if (item is not None ) or (item_email is not None):
                    ...
                    response = tank_cleanning.pack_assets.assetsController.eliminar(name,email)
                    if response == 0:
                        show_message_sucess("Successfully delete","Data removed successfully")
                        show_form_list_asset()
                    else:
                        show_message_error("Delete Error","Data error successfully removed")
                    
                    

        self.table_assets.horizontalHeader().setDefaultSectionSize(217)
        self.table_assets.horizontalHeader().setHighlightSections(False)
        self.table_assets.verticalHeader().setVisible(False)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_titulo_form.setText(_translate("MainWindow", "Assets"))
        self.lbl_text_form.setText(_translate("MainWindow", "Below is the list of all  Assets registered in the system"))
       
        self.btn_add_consumable.setText(_translate("MainWindow", "Add Asset "))
        self.btn_add_consumable.clicked.connect(lambda: show_form_add_asset())
        ...
        def show_form_add_asset():
            self.window = QtWidgets.QMainWindow()
            import tank_cleanning.pack_assets.asets_add
            self.ui = tank_cleanning.pack_assets.asets_add.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text())
            self.window.show()
            MainWindow.close()
