...
import tank_cleanning.pack_employee.tank_cleaning_enginer_add
...
"")    
        ...
        list_enginer = tank_cleanning.pack_employee.employeeController.listar_table()
        print(list_enginer)
        ...
        for row in list_enginer:
            ...
            self.table_employee.setItem(tablerow,0,QtWidgets.QTableWidgetItem(row[1]))
            self.table_employee.setItem(tablerow,1,QtWidgets.QTableWidgetItem(row[2]))
            self.table_employee.setItem(tablerow,2,QtWidgets.QTableWidgetItem(row[0]))
        ...
        def show_form_edit_engier(name,email,position):
            ...
            import tank_cleanning.pack_employee.tank_cleaning_enginer_edit
            self.ui = tank_cleanning.pack_employee.tank_cleaning_enginer_edit.Ui_MainWindow()
        ...
        def btn_clicked_delete():
            ...
            if isinstance(self.btn_remove_employee, QtWidgets.QPushButton):
                ...
                if (item is not None ) or (item_email is not None):
                    ...
                    response = tank_cleanning.pack_employee.employeeController.delete_data(name,email)
                    if response == 0:
    ...
    def retranslateUi(self, MainWindow):
        ...
        def show_form_add_engineer():
            self.window = QtWidgets.QMainWindow()
            import tank_cleanning.pack_employee.tank_cleaning_enginer_add
            self.ui = tank_cleanning.pack_employee.tank_cleaning_enginer_add.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text())
            self.window.show()
            MainWindow.close()
