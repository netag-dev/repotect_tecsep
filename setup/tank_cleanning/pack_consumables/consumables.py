...
"")
        ...
        self.btn_add_consumable = QtWidgets.QPushButton(self.frame)
        self.btn_add_consumable.setGeometry(QtCore.QRect(900, 240, 210, 31))
        self.btn_add_consumable.setStyleSheet("\n"
"\n"
"QPushButton#btn_add_consumable{\n"
"\n"
"border:none;\n"
"background-color:#044e42;\n"
"color:white;\n"
...
"}\n"
"\n"
"QPushButton#btn_add_consumable:hover{\n"
" background-color: #044e42;\n"
"border-radius: 6px;\n"
"transition: background-color 0.5s ease;\n"
"padding:8px;\n"
"}\n"
"\n"
"QPushButton#btn_add_consumable:pressed {\n"
" background-color: #044e42;\n"
"border-radius: 6px;\n"
"background-color: #033029;\n"
"padding:8px;\n"
" }\n"
"")
        self.btn_add_consumable.setIcon(icon7)
        self.btn_add_consumable.setObjectName("btn_add_consumable")
...
"")
        ...
        self.table_consumiveis = QtWidgets.QTableWidget(self.frame)
        self.table_consumiveis.setGeometry(QtCore.QRect(20, 320, 1091, 421))
        self.table_consumiveis.setAutoFillBackground(True)
        self.table_consumiveis.setStyleSheet("QTableWidget{\n"
"\n"
"\n"
"border:none;\n"
...
"")    
        ...
        self.table_consumiveis.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.table_consumiveis.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.table_consumiveis.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        self.table_consumiveis.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
        self.table_consumiveis.setTextElideMode(QtCore.Qt.ElideMiddle)
        self.table_consumiveis.setShowGrid(False)
        self.table_consumiveis.setGridStyle(QtCore.Qt.NoPen)
        self.table_consumiveis.setCornerButtonEnabled(False)
        self.table_consumiveis.setObjectName("table_consumiveis")
        self.table_consumiveis.setColumnCount(5) 
        self.table_consumiveis.setHorizontalHeaderLabels(["Consumable","Stock Quantity "," "," "," "])
        self.table_consumiveis.horizontalHeaderItem(0).setTextAlignment(0x0001)
        self.table_consumiveis.horizontalHeaderItem(1).setTextAlignment(0x0001)
        self.table_consumiveis.horizontalHeaderItem(2).setTextAlignment(0x0001)
        self.table_consumiveis.horizontalHeaderItem(3).setTextAlignment(0x0001)
        self.table_consumiveis.horizontalHeaderItem(4).setTextAlignment(0x0001)

        list_consumiveis = tank_cleanning.pack_consumables.consumablesController.listar_table()
        print(list_consumiveis)
        self.table_consumiveis.setRowCount(10)
        tablerow = 0

        for row in list_consumiveis:
            
            self.btn_edit_consumiveis = QtWidgets.QPushButton("Edit Consumiveis")
            self.btn_edit_consumiveis.setStyleSheet("\n" "\n" "QPushButton#btn_edit_consumiveis{\n" "\n" "border:none;\n" "background-color:#044e42;\n" "color:white;\n" "font-size:13px;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "text-align:rigth;\n" "}\n" "\n" "QPushButton#btn_edit_consumiveis:hover{\n" " background-color: #044e42;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "}\n" "\n" "QPushButton#btn_edit_consumiveis:pressed {\n" " background-color: #044e42;\n" "border-radius: 0px;\n" "background-color: #033029;\n" "padding:5px;\n" " }\n" "")
            self.btn_edit_consumiveis.setIcon(icon_edit)
            self.btn_edit_consumiveis.setObjectName("btn_edit_consumiveis")

            self.btn_remove_consumiveis = QtWidgets.QPushButton("Remove Consumiveis")
            self.btn_remove_consumiveis.setStyleSheet("\n" "\n" "QPushButton#btn_remove_consumiveis{\n" "\n" "border:none;\n" "background-color:#044e42;\n" "color:white;\n" "font-size:13px;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "text-align:rigth;\n" "}\n" "\n" "QPushButton#btn_remove_consumiveis:hover{\n" " background-color: #044e42;\n" "border-radius: 3px;\n" "transition: background-color 0.5s ease;\n" "padding:5px;\n" "}\n" "\n" "QPushButton#btn_remove_consumiveis:pressed {\n" " background-color: #044e42;\n" "border-radius: 0px;\n" "background-color: #033029;\n" "padding:5px;\n" " }\n" "")
            self.btn_remove_consumiveis.setIcon(icon_delete)
            self.btn_remove_consumiveis.setObjectName("btn_remove_consumiveis")
            
            

            self.table_consumiveis.setItem(tablerow,0,QtWidgets.QTableWidgetItem(row[1]))
            self.table_consumiveis.setItem(tablerow,1,QtWidgets.QTableWidgetItem(str(row[2])))
            self.table_consumiveis.setItem(tablerow,2,QtWidgets.QTableWidgetItem(row[0]))
            self.table_consumiveis.setCellWidget(tablerow,3,self.btn_edit_consumiveis)
            self.table_consumiveis.setCellWidget(tablerow,4,self.btn_remove_consumiveis)

            self.btn_edit_consumiveis.clicked.connect(lambda:btn_clicked_edit())
            self.btn_remove_consumiveis.clicked.connect(lambda:btn_clicked_delete())
        ...
        def show_form_edit_consumivies(name,quantidade):
            self.window = QtWidgets.QMainWindow()
            import tank_cleanning.pack_consumables.consumables_edit as edit
            self.ui = edit.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text(),name,quantidade)
            self.window.show()
            MainWindow.close()

        def show_form_list_consumiveis():
            self.window = QtWidgets.QMainWindow()
            import tank_cleanning.pack_consumables.consumables as lista
            self.ui = lista.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text())
            self.window.show()
            MainWindow.close()

        def btn_clicked_edit():
            self.btn_edit_consumiveis = self.btn_edit_consumiveis.sender()
            if isinstance(self.btn_edit_consumiveis, QtWidgets.QPushButton):
                index = self.table_consumiveis.indexAt(self.btn_edit_consumiveis.pos())
                row = index.row()
                col = index.column()
                ...
                item_position = self.table_consumiveis.item(row,col_position)
                
                item = self.table_consumiveis.item(row,col)

                item_quatidade = self.table_consumiveis.item(row,col_email)
                
                if (item is not None ) or (item_position is not None):
                    
                    name = item.text()
                    quatidade = item_quatidade.text()
                    show_form_edit_consumivies(name,quatidade)

          

        def btn_clicked_delete():
            self.btn_remove_consumiveis = self.btn_remove_consumiveis.sender()
            if isinstance(self.btn_remove_consumiveis, QtWidgets.QPushButton):

                index = self.table_consumiveis.indexAt(self.btn_remove_consumiveis.pos())
                row = index.row()
                col = index.column()
                ...
                item_quantity = self.table_consumiveis.item(row,col_email)
                
                if (item is not None ) or (item_quantity is not None):
                    
                    name = item.text()
                    quantity = item_quantity.text()

                    print(name,quantity)

                    response = tank_cleanning.pack_consumables.consumablesController.eliminar(name,quantity)
                    if response == 0:
                        show_message_sucess("Successfully delete","Data removed successfully")
                        show_form_list_consumiveis()
                    else:
                        show_message_error("Delete Error","Data error successfully removed")
                    
                    

        self.table_consumiveis.horizontalHeader().setDefaultSectionSize(217)
        self.table_consumiveis.horizontalHeader().setHighlightSections(False)
        self.table_consumiveis.verticalHeader().setVisible(False)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_titulo_form.setText(_translate("MainWindow", "Consumables"))
        self.lbl_text_form.setText(_translate("MainWindow", "Below is the list of all  Consumables registered in the system"))
       
        self.btn_add_consumable.setText(_translate("MainWindow", "Add Consumable "))
        self.btn_add_consumable.clicked.connect(lambda: show_form_add_consumable())
       
        self.btn_list_filtration_report.setText(_translate("MainWindow","List Tank Cleaning"))
        self.btn_list_filtration_report.clicked.connect(lambda:show_form_list_tank_cleaning())

        def show_form_list_tank_cleaning():
            self.window = QtWidgets.QMainWindow()  
        ...
        def show_form_add_consumable():
            self.window = QtWidgets.QMainWindow()
            import tank_cleanning.pack_consumables.consumables_add
            self.ui = tank_cleanning.pack_consumables.consumables_add.Ui_MainWindow()
            self.ui.setupUi(self.window,self.lbl_user_logado.text())
            self.window.show()
            MainWindow.close()
